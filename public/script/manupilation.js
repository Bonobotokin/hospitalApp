...
for (let y = 0; y < tableLigne.length; y++) {
  ...
  clickContent.addEventListener('click', function () {
    ...
    for (let j = 0; j < lignes.length; j++) {
      div.innerHTML = `
        <input type="hidden" name="produits[${j}][produits_id]" value="${id}" class="form-control" id="id">
        <input type="text" name="produits[${j}][designationProduits]" value="${designationProduits}" class="form-control mb-2 mr-1 col-lg-3" id="designationProduits">
        <input type="text" name="produits[${j}][type]" value="${type}" class="form-control mb-2 mr-1 col-lg-3" id="type">
        <input type="hidden" name="produits[${j}][prix]" value="${prix}" class="form-control mb-2 mr-1 col-lg-2" id="prix">
        <input type="text" name="produits[${j}][quantite]" placeholder="quantite" class="form-control mb-2 mr-1 col-lg-3" id="quantite">
      `;
